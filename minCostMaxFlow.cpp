...
int d(){
    ...
    //FOR MF, YOU CAN USE JUST A QUEUE OF INTS
...
int update(int &cst){
    ...
    //cst+= NOT NEEDED FOR MF
    loopP() {nodes[p][c] -= ret; nodes[c][p] += ret; cst += cost[p][c] * ret;}
    return ret;
}
...
int maxFlow(int& c){
    ...
    f(k,nnodes)
        f(i,nnodes){
                ...
                int to = j->first;
                //make sure the edge is not a backwards residual edge
                //in the graph.
                if(j->second>0)
